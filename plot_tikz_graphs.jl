...
so maybe include a way of setting markers and colours easily?
    ...
    save_paths_tikz(g::AbstractGraph,
                             paths,
...
"""
    save_paths_tikz_edg(g::AbstractGraph,
                             edges_array,
                             node_coords::Array{Float64,2},
                             filename::String;
                             imp_nodes=[],
                             ...
                             standalone_doc=true)
"""
function save_graph_tikz_edg(g::AbstractGraph,
                             edges_array,
                             node_coords::Array{Float64,2},
                             filename::String;
                             imp_nodes=[],
    ...
    if m != 0   #If there are no edges, adding an empty scope is likely to cause problems
        drawing *= "\\begin{scope}\n"
        for (k,edge) in enumerate(edges(g))
            s = edge.src
            t = edge.dst
            if edge_labels == true
                l_pos = s > t ? "above" : "below"
                drawing *= "\\draw[thick,opacity=0.1] ($(s).center) -- ($(t).center) node [midway,$(l_pos),opacity=0.9] {$(k)};\n"
            else
                drawing *= "\\draw[thick,opacity=0.1] ($(s).center) -- ($(t).center);\n"
            end
        end
